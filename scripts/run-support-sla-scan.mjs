...
const response = await fetch(`${baseUrl}/api/internal/platform/support-sla-scan`, {
  method: "POST",
...
if (!response.ok) {
  console.error(`Support SLA scheduler failed with HTTP ${response.status}: ${body.slice(0, 1000)}`);
  process.exit(1);
}
