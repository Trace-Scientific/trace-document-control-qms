...
ALTER TABLE "DocumentVersion"
ADD CONSTRAINT "DocumentVersion_number_positive" CHECK ("versionNumber" > 0),
ADD CONSTRAINT "DocumentVersion_lock_nonnegative" CHECK ("lockVersion" >= 0),
ADD CONSTRAINT "DocumentVersion_revision_label_nonempty" CHECK (length(btrim("revisionLabel")) > 0),
ADD CONSTRAINT "DocumentVersion_change_summary_nonempty" CHECK (length(btrim("changeSummary")) > 0);
