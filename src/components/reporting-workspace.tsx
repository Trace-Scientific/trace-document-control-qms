...
export function ReportingWorkspace({canManage,canExport}:{canManage:boolean;canExport:boolean}){
  ...
  return <section className="workspace-section reporting-workspace" aria-labelledby="reporting-workspace-title">
    ...
    {canManage&&<div className="card reporting-card"><form className="admin-form reporting-form" onSubmit={e=>{e.preventDefault();void createDefinition();}}><h3 className="reporting-form-span">Governed report definitions</h3><p className="reporting-form-span reporting-help">Create definitions only from approved server-backed report sources.</p><label>Code<input value={newCode} onChange={e=>setNewCode(e.target.value)} /></label><label>Name<input value={newName} onChange={e=>setNewName(e.target.value)} /></label><label className="reporting-form-wide">Description<input value={newDescription} onChange={e=>setNewDescription(e.target.value)} /></label><label>Approved source<select value={newSource} onChange={e=>setNewSource(e.target.value as SourceKey)}>{(Object.keys(sourceLabels) as SourceKey[]).map(key=><option key={key} value={key}>{sourceLabels[key]}</option>)}</select></label><button type="submit" disabled={!newCode.trim()||!newName.trim()}>Create governed report</button></form></div>}

    <div className="card reporting-card"><div className="admin-form reporting-form"><h3 className="reporting-form-span">Run governed report</h3><label>Report<select value={selected} onChange={e=>chooseReport(e.target.value)}><option value="">Select report</option>{definitions.map(d=><option key={d.id} value={d.id}>{d.code} — {d.name}</option>)}</select></label><label>Status filter<select value={status} onChange={e=>{setStatus(e.target.value);setSavedViewId("");}} disabled={!selectedDefinition}><option value="">All approved statuses</option>{allowedStatuses.map(value=><option key={value} value={value}>{value}</option>)}</select></label><label>Personal saved view<select value={savedViewId} onChange={e=>chooseSavedView(e.target.value)} disabled={!selected}><option value="">Use current filter</option>{savedViews.map(view=><option key={view.id} value={view.id}>{view.name}</option>)}</select></label><button type="button" onClick={execute} disabled={!selected}>Run report</button></div>
      <div className="admin-form reporting-form reporting-saved-view-form"><label className="reporting-form-wide">Save current filter as<input value={savedViewName} onChange={e=>setSavedViewName(e.target.value)} disabled={!selected}/></label><button type="button" onClick={saveView} disabled={!selected||!savedViewName.trim()}>Save personal view</button><button type="button" className="reporting-secondary-action" onClick={deleteView} disabled={!savedViewId}>Delete selected view</button></div>
    </div>

    {message&&<p role="status">{message}</p>}

    <div className="reporting-history-grid">
      <section className="card reporting-history-card" aria-labelledby="reporting-execution-history"><h3 id="reporting-execution-history">Execution history</h3>{executions.length===0?<p className="reporting-empty-state">No executions.</p>:<ul className="reporting-list">{executions.map(e=><li key={e.id}><span><strong>{e.reportCode}</strong> — {e.rowCount} rows — {new Date(e.executedAt).toLocaleString()}</span>{canManage&&<button type="button" onClick={()=>finalize(e.id)}>Finalize</button>}</li>)}</ul>}</section>
      <section className="card reporting-history-card" aria-labelledby="reporting-finalized-reports"><h3 id="reporting-finalized-reports">Finalized reports</h3>{finalized.length===0?<p className="reporting-empty-state">No finalized reports.</p>:<ul className="reporting-list">{finalized.map(f=><li key={f.id}><span><strong>{f.reportCode}</strong> — {f.rowCount} rows — {new Date(f.finalizedAt).toLocaleString()}</span>{canExport&&<a className="reporting-export-link" href={`/api/reporting/finalized?finalizedReportId=${encodeURIComponent(f.id)}`}>Export CSV</a>}</li>)}</ul>}</section>
    </div>
  </section>;
}
