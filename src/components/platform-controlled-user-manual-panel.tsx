...
type Detail = { manual: Manual; sections:Array<{id:string;sectionCode:string;title:string;latestRevisionNumber:number|null;revisionCount:number}>; releases:Array<{id:string;version:string;status:string;effectiveAt:string;sectionCount:number;publishedAt:string|null}> };

export function PlatformControlledUserManualPanel() {
  const [manuals,setManuals]=useState<Manual[]>([]); const [detail,setDetail]=useState<Detail|null>(null); const [error,setError]=useState<string|null>(null);
  ...
  return <div className={styles.grid}>
    ...
    <article className={styles.card}>
      ...
      {!detail ? <p>Select a manual to inspect its controlled composition and release state.</p> : <>
        ...
        {detail.releases.map(r=><p key={r.id}><strong>v{r.version}</strong> · {r.status} · effective {new Date(r.effectiveAt).toLocaleString()} · {r.sectionCount} frozen sections</p>)}
      </>}
