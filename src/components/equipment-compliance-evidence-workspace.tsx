...
export function EquipmentComplianceEvidenceWorkspace({today}:{today:string}){
  ...
  return <section className="card form-stack equipment-register-style">
    ...
    <form className="admin-form equipment-admin-form" onSubmit={recordEvidence}>
      ...
      <label className="equipment-schedule-date-field">Evidence type<select value={eventType} onChange={event=>{setEventType(event.target.value as EventType);setMessage("");setEvidenceFileId("");}}><option value="CALIBRATED">Calibration completed</option><option value="MAINTENANCE">Preventive maintenance completed</option></select></label>
