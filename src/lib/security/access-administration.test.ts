...
describe("access administration boundary", () => {
  it("requires an authenticated administration grant for reads and changes", () => { for (const source of [queryRoute, commandRoute]) { expect(source).toContain("authenticateRequest(request)"); expect(source).toContain('permission: \"administration.manage\"'); expect(source).toContain('error.message === \"Access denied\"'); expect(source).toContain("status: 403"); } });
  ...
  it("prevents user-specific administration responses from being cached across sessions", () => { expect(queryRoute).toContain('cache-control'); expect(queryRoute).toContain('private, no-store'); expect(queryRoute).toContain('vary'); expect(queryRoute).toContain('Cookie'); });
