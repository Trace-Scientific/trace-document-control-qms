...
import { DocumentCommandError, DocumentCommandService, DocumentConcurrencyError } from "@/lib/documents/service";
...
const updateDraft=z.object({operation:z.literal("UPDATE_DRAFT"),versionId:uuid,expectedLockVersion:z.number().int().nonnegative(),contentText:z.string().trim().min(1).max(1_000_000),changeSummary:z.string().trim().min(1).max(4000)});
const commandSchema = z.discriminatedUnion("operation", [createDraft, updateDraft, transition]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = command.operation === "CREATE_DRAFT"
      ? await service.createDraft(context, { ...command, organizationId: context.organizationId, contentHash: createHash("sha256").update(command.contentText,"utf8").digest("hex") })
      : command.operation === "UPDATE_DRAFT"
        ? await service.updateDraft(context,{...command,organizationId:context.organizationId,contentHash:createHash("sha256").update(command.contentText,"utf8").digest("hex")})
        : await service.transition(context, { ...command, organizationId: context.organizationId });
