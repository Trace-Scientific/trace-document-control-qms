...
export async function POST(request:NextRequest){
  try{
    ...
    return NextResponse.json({data:await service.assignRole(context,schema.parse(await request.json()))},{status:201});
  }catch(error){
    const message=error instanceof Error?error.message:"Platform role assignment failed.";
