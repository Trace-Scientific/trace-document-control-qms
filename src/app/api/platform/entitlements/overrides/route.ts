...
const schema = z.object({
  ...
  decision: z.enum(["ENABLE", "DISABLE"]),
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ data: await service.createEntitlementOverride(context, schema.parse(await request.json())) }, { status: 201 });
