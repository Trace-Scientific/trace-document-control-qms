...
const schema = z.object({ body: z.string().min(1), changeSummary: z.string().min(1).max(1000), expectedLockVersion: z.number().int().positive(), reason: z.string().min(1).max(1000) });

export async function POST(request: NextRequest, { params }: { params: Promise<{ articleId: string }> }) {
  try {
    const context = await authenticatePlatformRequest(request);
    const { articleId } = await params;
    return NextResponse.json({ data: await service.addArticleRevision(context, { articleId, ...schema.parse(await request.json()) }) }, { status: 201 });
  } catch (error) {
